...
async function analyzeLeetCodeProblemWithGemini({
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return buildGeminiFallbackResult({
        reason: `Gemini API error ${response.status}: ${summarizeGeminiError(errorText)}`,
        fallbackComplexity,
        fallbackCompanyTags,
        model,
      });
...
Return only valid JSON:
{
  ...
  try {
    ...
    if (!response.ok) {
      const errorText = await response.text();
      return buildCompanyFallbackResult({
        reason: `Gemini API error ${response.status}: ${summarizeGeminiError(errorText)}`,
        fallbackCompanyTags,
        model,
      });
...
function buildGeminiFallbackResult({
  reason,
  fallbackComplexity,
  fallbackCompanyTags,
  model,
  ...
  return {
    ...
    model,
    reason,
    analysis: normalizeGeminiAnalysis({
      timeComplexity: fallbackComplexity?.timeComplexity,
      spaceComplexity: fallbackComplexity?.spaceComplexity,
      companyTags: fallbackCompanyTags,
      confidence: 'low',
    }),
  };
}

function buildCompanyFallbackResult({ reason, fallbackCompanyTags, model }) {
  return {
    success: false,
    model,
    reason,
    companyTags: normalizeCompanyTags(fallbackCompanyTags),
    companyTagReasoning:
      'Using fallback company tags because Gemini is unavailable.',
    confidence: 'low',
  };
}
